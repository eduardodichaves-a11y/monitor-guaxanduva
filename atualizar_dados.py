...
# =========================================================
# HISTÓRICO DE AUTOVALIDAÇÃO #120
# =========================================================
...
def carregar_historico_validacao():
    try:
        ...
        print(
            "Aviso: histórico anterior não pôde ser lido: "
            + str(e)
        )
        return []
...
def registrar_historico_validacao(dados):
    ...
    Guarda uma série temporal compacta da autovalidação do radar.
    Não define limiar de aprovação e não libera ETA.

    Para evitar duplicação quando duas execuções enxergam o mesmo
    quadro do radar, a chave lógica é o horário do último quadro.
    """
    ...
    if chave:
        registros = [
            ...
            if item.get("horario_ultimo_quadro") != chave
        ]
    ...
    erros_medios = [
        item.get("erro_medio_km")
        for item in registros
        if isinstance(item.get("erro_medio_km"), (int, float))
    ]

    total_previsoes = sum(
        item.get("total_previsoes_testadas") or 0
        for item in registros
    )

    resumo = {
        "execucoes_registradas": len(registros),
        "execucoes_com_erro_medio": len(erros_medios),
        "total_previsoes_testadas_somadas": total_previsoes,
        "media_dos_erros_medios_km": (
            round(sum(erros_medios) / len(erros_medios), 2)
            if erros_medios
            else None
        ),
    ...
    historico = {
        ...
        "politica_retencao": (
            "Mantém no máximo 2880 quadros únicos de radar; "
            "aproximadamente 30 dias se houver atualização a cada 15 minutos."
        ),
        "limite_aprovacao_definido": False,
        "usado_para_liberar_eta": False,
        "validado_para_eta": False,
...
def rastrear(quadros):
    ...
    return {
        ...
        "observacao":
            (
                "O #120 mantém identidade temporal "
...
def analisar_interceptacao(
    ...
    base = {
        ...
        "versao_metodo":
            "#120_identidade_temporal_multivetorial",
    ...
    try:
        ...
        if principal:
            movimento = {
                ...
                "metodo":
                    "identidade_temporal_multivetorial_#120",
...
def main():
    ...
    print(
        "historico_validacao.json atualizado com sucesso: "
        + str(historico["resumo"]["execucoes_registradas"])
        + " registros"
    )
